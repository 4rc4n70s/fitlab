  ...
  try {
    ...
    if (!paymentId) {
      if (!process.env.MP_ACCESS_TOKEN) {
        ...
        return NextResponse.json({ success: true, credits: profile?.boilerplate_credits ?? 5 });
      }
      ...
      if (!searchRes.ok) {
        ...
        return NextResponse.json({ 
          success: true, 
          credits: profile?.boilerplate_credits ?? 5, 
      ...
      return NextResponse.json({
        success: true,
        credits: updatedProfile?.boilerplate_credits ?? 5,
        newlyCredited,
    ...
    if (insertErr) {
      ...
      if (insertErr.code === '42P01') {
        console.warn('Tabla boilerplate_processed_payments no existe. Por favor ejecuta el script de migración SQL en Supabase.');
