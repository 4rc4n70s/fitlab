  ...
  try {
    ...
    if (type === 'payment' && paymentId) {
      ...
      if (paymentData.status === 'approved') {
        ...
        // Regla de pruebas: 100 créditos por $100 ARS
        let creditsToAdd = 0;
        ...
        if (userId && creditsToAdd > 0) {
          // 3. Prevenir doble acreditación registrando el pago en la tabla de Boilerplate
          if (process.env.SUPABASE_SERVICE_ROLE_KEY) {
            ...
            if (insertErr && insertErr.code === '23505') {
              console.log(`[Webhook] Pago ${paymentId} ya fue procesado anteriormente en Boilerplate. Ignorando.`);
          ...
          console.log(`[Acreditado] ${creditsToAdd} créditos de Boilerplate al usuario ${userId} por pago ID ${paymentId}`);
        }
      }
    }
