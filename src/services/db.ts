...
/**
 * ⚡ Fit Lab Custom Lightweight ORM
...
export const db = {
  profiles: {
     ...
     */
    async create(data: { id: string; credits?: number }, options?: { admin?: boolean }): Promise<Profile> {
      const client = getClient(options?.admin);
    ...
    async incrementCredits(userId: string, amount: number, options?: { admin?: boolean }): Promise<void> {
      ...
      try {
        const { error: rpcErr } = await client.rpc('increment_user_credits', {
          user_id: userId,
          amount_to_add: amount
        });
        
        if (!rpcErr) return;
        console.warn('RPC increment_user_credits no disponible, usando fallback directo:', rpcErr.message);
      } catch (e) {
        console.warn('Excepción al ejecutar RPC increment_user_credits:', e);
      }

      // Fallback directo: Obtener saldo actual, sumar y actualizar
      const profile = await this.findUnique(userId, options);
      const currentCredits = profile?.credits || 0;
      await this.update(userId, { credits: currentCredits + amount }, options);
    },
    ...
    async decrementCredits(userId: string, amount: number, options?: { admin?: boolean }): Promise<void> {
      ...
      try {
        const { error: rpcErr } = await client.rpc('decrement_user_credits', {
          user_id: userId,
          amount_to_subtract: amount
        });
        
        if (!rpcErr) return;
        console.warn('RPC decrement_user_credits no disponible, usando fallback directo:', rpcErr.message);
      } catch (e) {
        console.warn('Excepción al ejecutar RPC decrement_user_credits:', e);
      }
